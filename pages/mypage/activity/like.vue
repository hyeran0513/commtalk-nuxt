...
// 데이터 변환 로직
const transformGridData = (likesPosts) => {
  return likesPosts.map((req) => ({
    postId: req.postId,
...
// gridData 설정
const setGridData = (likesPosts) => {
  gridData.value = transformGridData(likesPosts);
}
...
onMounted(async () => {
  ...
  if (likes.value.posts) {
    setGridData(likes.value.posts);
  }
});
