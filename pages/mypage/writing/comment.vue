...
// 데이터 변환 로직
const transformGridData = (commentsPosts) => {
  return commentsPosts.map((req) => ({
    postId: req.postId,
...
// gridData 설정
const setGridData = (commentsPosts) => {
  gridData.value = transformGridData(commentsPosts);
}
...
onMounted(async () => {
  ...
  if (comments.value.posts) {
    setGridData(comments.value.posts);
  }
});
